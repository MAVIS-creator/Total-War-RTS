...
if (map.playerCount !== 2 || map.spawnPoints.length !== 2 || map.oreFields.length !== 3)
  throw new Error('Expected Frozen Front to be a complete 1v1 map.');
if (!isBlocked(map.terrain, { x: 225, y: 125 })) throw new Error('Expected authored blocked terrain to be reported.');
if (isBlocked(map.terrain, map.spawnPoints[0]?.position ?? { x: 0, y: 0 }))
  throw new Error('Expected map spawn to be passable.');
try {
...
if (simulation.drainEvents().filter((event) => event.type === 'unit-queued').length !== 1)
  throw new Error('Expected one unit-queued event.');
...
if (player.population.used !== 2 || player.population.reserved !== 0)
  throw new Error('Expected queued population to transfer to used population.');
if (simulation.snapshot().units.length !== 1) throw new Error('Expected one completed tank.');
if (simulation.drainEvents().filter((event) => event.type === 'unit-completed').length !== 1)
  throw new Error('Expected one unit-completed event.');
...
if (!researchSimulation.beginResearch('research-player', 'tech2').accepted)
  throw new Error('Expected Tech 2 research to start.');
if (researchSimulation.drainEvents().filter((event) => event.type === 'research-started').length !== 1)
  throw new Error('Expected a research-started event.');
researchSimulation.advance(12);
const researchPlayer = researchSimulation.snapshot().players[0];
if (!researchPlayer || researchPlayer.techLevel !== 2 || researchPlayer.research)
  throw new Error('Expected Tech 2 research to complete.');
if (researchPlayer.economy.orePerSecond !== 1_300) throw new Error('Expected Tech 2 HQ income progression.');
if (researchSimulation.drainEvents().filter((event) => event.type === 'research-completed').length !== 1)
  throw new Error('Expected a research-completed event.');
...
if (!movementSimulation.queueUnit('move-player', movementFactory, 'scout').accepted)
  throw new Error('Expected scout queue for movement test.');
...
if (!movementSimulation.issueMove('move-player', [movingUnit.id], { x: 500, y: 500 }).accepted)
  throw new Error('Expected move order to be accepted.');
if (movementSimulation.issueMove('move-player', [movingUnit.id], { x: 225, y: 125 }).accepted)
  throw new Error('Expected blocked move destination to be rejected.');
movementSimulation.advance(4);
const movedUnit = movementSimulation.snapshot().units[0];
if (!movedUnit || Math.hypot(movedUnit.position.x - 500, movedUnit.position.y - 500) > 0.01 || movedUnit.destination)
  throw new Error('Expected unit to complete its move order.');
if (movementSimulation.drainEvents().filter((event) => event.type === 'move-issued').length !== 1)
  throw new Error('Expected one move-issued event.');
...
if (!combatSimulation.queueUnit('attacker', combatFactory, 'tank').accepted)
  throw new Error('Expected tank queue for combat test.');
combatSimulation.advance(3);
const attacker = combatSimulation.snapshot().units[0];
if (!attacker || !combatSimulation.issueAttack('attacker', [attacker.id], targetHq).accepted)
  throw new Error('Expected attack order to be accepted.');
combatSimulation.advance(2);
const damagedHq = combatSimulation.snapshot().buildings.find((building) => building.id === targetHq);
if (!damagedHq || damagedHq.health >= damagedHq.maxHealth)
  throw new Error('Expected attack order to damage the target.');
const combatEvents = combatSimulation.drainEvents();
if (combatEvents.filter((event) => event.type === 'attack-issued').length !== 1)
  throw new Error('Expected one attack-issued event.');
if (combatEvents.filter((event) => event.type === 'projectile-fired').length === 0)
  throw new Error('Expected projectile firing events.');
...
if (!autoCombatSimulation.queueUnit('auto-attacker', autoFactory, 'tank').accepted)
  throw new Error('Expected tank queue for auto-targeting test.');
...
if (!autoDamagedHq || autoDamagedHq.health >= autoDamagedHq.maxHealth)
  throw new Error('Expected nearby enemy target to be acquired automatically.');
